...
class UserRegistrationForm(UserCreationForm):
    ...
    first_name = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={
                "class": INPUT_CSS,
                "placeholder": "Adınız",
                "pattern": r"[A-Za-zÇÖÜĞİŞçöüğiş\s-]+",
                "oninput": r'this.value = this.value.replace(/[^A-Za-zÇÖÜĞİŞçöüğiş\s-]/g, "")',
            }
        ),
    )
    ...
    def clean_tc_id(self):
        ...
        if not tc_id or not tc_id.isdigit():
            raise forms.ValidationError("TC sadece rakam olmalıdır.")

        if tc_id and len(tc_id) != 11:
